...
function Gallary() {
    ...
    async function imageGallaryList() {
        ...
        const response = await axios.get(`https://api.slingacademy.com/v1/sample-data/photos?limit=20&offset=${gallaryState.offset}`)
        ...
        <div className="image-gallary-wrapper">
            {
                ...
                ? 'Loading...' 
                : gallaryState.photoList.map(photo => <Image url={photo.url} key={photo.id} id={photo.id} /> )
            }
        </div>
        <div className="controller-wrapper">
            <button 
                className="prev-btn" 
                onClick={() => setGallaryState((state) => ({ ...state, offset: state.offset - 20 }))} 
